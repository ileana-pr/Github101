# 🐙 GitHub for Vibe Coders

Welcome! This repo contains all student and instructor resources for the **GitHub for Vibe Coders** workshop series.

The series is split into two sessions — you can attend one or both:

---

## 📚 Workshop 1 — Introduction

**Duration:** ~60 minutes | **Level:** Complete beginner

**What you'll learn:** What GitHub is, how to set up authentication, create your first repo, and make your first open source contribution.

| File | What it is |
|---|---|
| [📅 Agenda](./introduction/student-resources/agenda.md) | What we'll cover — read this first |
| [📋 Pre-Requisite Checklist](./introduction/student-resources/ready-up-checklist.md) | Complete this **before** the session |
| [🗒️ Cheatsheet](./introduction/student-resources/cheatsheet.md) | Keep this open during the session — every command we run |
| [📖 Lesson Plan](./lesson-plans/01-introduction-lesson-plan.md) | Full session outline (instructors) |
| [🎙️ Instructor Guide](./lesson-plans/01-introduction-instructor-guide.md) | Live delivery guide with timestamps (instructors) |

---

## 🔐 Workshop 2 — Security

**Duration:** ~60 minutes | **Level:** Requires Workshop 1

**What you'll learn:** `.gitignore`, `.env` files, API key safety, SSH key hygiene, recovering from a leaked secret, and hardening your GitHub account.

| File | What it is |
|---|---|
| [📅 Agenda](./security/student-resources/agenda.md) | What we'll cover — read this first |
| [📋 Pre-Requisite Checklist](./security/student-resources/ready-up-checklist.md) | Complete this **before** the session |
| [🗒️ Cheatsheet](./security/student-resources/cheatsheet.md) | Keep this open during the session — every command we run |
| [📖 Lesson Plan](./lesson-plans/02-security-lesson-plan.md) | Full session outline (instructors) |
| [🎙️ Instructor Guide](./lesson-plans/02-security-instructor-guide.md) | Live delivery guide with timestamps (instructors) |

---

## 🎓 Student List

Add your name during class — it's your first pull request!

[🎓 STUDENTS.md](./STUDENTS.md)

---

## 👀 How to View a File in This Repo

1. Click any link in the tables above
2. The file will open right here in GitHub — no downloading needed!

> **Pro tip:** Click the **Raw** button on any file to see the plain text version — great for copying commands.

---

Made with ❤️ by [@adigitaltati](https://x.com/adigitaltati) for [VDAO](https://x.com/joinvdao)
